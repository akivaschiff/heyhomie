...
if (!SPREADSHEET_ID) throw new Error("PANTRY_SHEET_ID environment variable is required");
const ITEMS_SHEET = "Items";
const LOG_SHEET = "Log";

// Types
type Level = "out" | "low" | "good" | "stocked";
...
async function getItems(): Promise<PantryItem[]> {
...
async function findItem(name: string): Promise<PantryItem | null> {
  const items = await getItems();
  return items.find((i) => i.name.toLowerCase() === name.toLowerCase()) || null;
}

// Helper: update item row
async function updateItem(row: number, data: Partial<PantryItem>): Promise<void> {
  const sheets = await getSheetsClient();
...
async function appendItem(name: string, unit: string | null, quantity: number | null, level: Level, par: number | null): Promise<void> {
  const sheets = await getSheetsClient();
...
// Helper: log action
async function logAction(item: string, action: string, quantityChange: number | null, note: string, rawInput: string): Promise<void> {
  const sheets = await getSheetsClient();
  await sheets.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${LOG_SHEET}!A:F`,
...
function deriveLevel(quantity: number | null, par: number | null): Level {
  if (quantity === null || par === null) return "good";
  if (quantity === 0) return "out";
  if (quantity <= par * 0.25) return "low";
  if (quantity >= par) return "stocked";
  return "good";
}
...
server.tool(
  "get_item",
  "Get a specific pantry item by name",
...
server.tool(
  ...
  async ({ name, unit, quantity, level, par, rawInput }) => {
    ...
    if (existing) return { content: [{ type: "text", text: `Item "${name}" already exists` }] };
...
server.tool(
  "use_item",
  "Record using/consuming an item (decrements quantity or lowers level)",
  {
    name: z.string().describe("Item name"),
...
server.tool(
  "restock_item",
  "Record restocking an item (adds quantity or raises level)",
  {
    name: z.string().describe("Item name"),
  ...
  async ({ name, quantity, level, rawInput }) => {
    const item = await findItem(name);
    if (!item) return { content: [{ type: "text", text: `Item "${name}" not found. Use add_item first.` }] };
...
server.tool(
  ...
  async () => {
    ...
    if (low.length === 0) return { content: [{ type: "text", text: "All items are stocked!" }] };
...
server.tool(
  "set_item_out",
  "Quickly mark an item as out of stock",
  {
    name: z.string().describe("Item name"),
    rawInput: z.string().describe("Original voice input that triggered this action"),
  },
